...
public class WebSecurityConfig {
  ...
  UserDetailsService userDetailsService() { 
	...
	SecurityFilterChain configureHttpSecurity(HttpSecurity http) throws Exception {
		 ...
		 * .requestMatchers("/states/list_by_country/**").hasAnyAuthority("Admin",
		 * "Salesperson") .requestMatchers("/users/**", "/settings/**", "/countries/**",
		 * "/states/**").hasAuthority("Admin") .requestMatchers("/categories/**",
		 ...
		 * 
		 * .requestMatchers("/orders", "/orders/", "/orders/page/**",
		 ...
		 * .usernameParameter("email") .permitAll() ) .logout(logout ->
		 * logout.permitAll()); http .rememberMe(rem -> rem
		 * .key("AbcDefgHijKlmnOpqrs_1234567890") .tokenValiditySeconds(7 * 24 * 60 *
		 * 60) ); http.headers(headers -> headers.frameOptions(f -> f.sameOrigin()));
		 */
	...
	/*
	 * @Bean WebSecurityCustomizer configure() throws Exception { return (web) ->
